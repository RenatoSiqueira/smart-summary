...
describe('OpenAIService', () => {
  ...
  describe('streamSummarize', () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          headers: expect.objectContaining({
            ...
            // OpenAI doesn't use HTTP-Referer or X-Title headers
          }),
    ...
    it('should calculate cost correctly for gpt-4', async () => {
      ...
      const allChunks = await firstValueFrom(
        service.streamSummarize(mockText, { model: 'gpt-4' }).pipe(toArray()),
      );
    ...
    it('should calculate cost correctly for gpt-4-turbo', async () => {
      ...
      const allChunks = await firstValueFrom(
        service
          .streamSummarize(mockText, { model: 'gpt-4-turbo' })
          .pipe(toArray()),
      );
    ...
    describe('OpenAI-specific behavior', () => {
      it('should not include HTTP-Referer or X-Title headers', async () => {
        const mockChunks = ['data: [DONE]\n\n'];
        const mockStream = createMockReadableStream(mockChunks);
        const mockResponse = createMockResponse(true, mockStream);
        (global.fetch as jest.Mock).mockResolvedValue(mockResponse);
      ...
      it('should use OpenAI API URL', async () => {
        const mockChunks = ['data: [DONE]\n\n'];
        const mockStream = createMockReadableStream(mockChunks);
        const mockResponse = createMockResponse(true, mockStream);
        (global.fetch as jest.Mock).mockResolvedValue(mockResponse);
